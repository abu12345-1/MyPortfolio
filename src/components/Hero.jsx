...
// Terminal build log — mirrors a real Maven/Spring Boot workflow instead
// of a frontend-only one, matching the resume's backend focus.
const LOG_LINES = [
  { text: "$ mvn clean install -DskipTests", type: "cmd" },
  { text: "[INFO] Compiling 42 source files...", type: "out" },
  { text: "[INFO] BUILD SUCCESS", type: "pass" },
  { text: "$ java -jar order-service.jar", type: "cmd" },
  { text: "✓ Spring Boot started on :8080", type: "pass" },
  { text: "$ whoami", type: "cmd" },
  { text: "Abubucker — Java Full Stack Developer", type: "out" },
];
...
function useTypedLog(lines, speed = 24, lineDelay = 340) {
  const [rendered, setRendered] = useState([""]);
  const reduceMotion = useReducedMotion();
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center w-full">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4">
            <motion.a
              href="/Abubucker_Resume_JavaDeveloper.pdf"
              whileHover={{ y: -3 }}
              whileTap={{ scale: 0.96 }}
              className="bg-[#10B981] text-white px-6 py-3 rounded-lg font-medium shadow-md shadow-emerald-900/10 hover:shadow-lg transition-shadow"
            >
        ...
        >
          ...
          >
            ...
            <div className="bg-[#0B0E14] px-5 py-6 font-mono text-[13px] md:text-sm min-h-[240px]">
              {typed.map((text, i) => {
                const meta = LOG_LINES[i];
                if (!meta) return null;
                const color =
